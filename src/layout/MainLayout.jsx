import { useRef } from "react";
import { Outlet, UNSAFE_useScrollRestoration } from "react-router-dom";
import Navbar from "../component/Navbar";
import TopNavbar from "../component/TopNavbar";
import ScrollRestoration from "../component/ScrollRestoration";
import Footer from "../component/Footer";
import { Heart } from "lucide-react";
import useProfileCompletionGuard from "../hooks/UseProfileCompletionGuard";
// import MobilePageTransition from "../component/MobilePageTransition";

const MainLayout = () => {
    const scrollRef = useRef(null);

    // Nudges users with an incomplete profile (missing bloodGroup/district/phone/gender)
    // back to /complete-profile every 30s, no matter where they navigate to.
    useProfileCompletionGuard();

    return (
        <div className="bg-[#F2F4F7] min-h-screen">
            <div className="flex min-h-screen flex-col">
                <div className="fixed w-full z-20">
                    <TopNavbar />
                </div>

                <div ref={scrollRef} className="flex-1 overflow-y-auto md:pb-0 pt-14 ">
                    <ScrollRestoration scrollContainerRef={scrollRef} />
                    <Outlet />
                    <button className="fixed md:bottom-6 bottom-20 right-6 flex h-14 w-14 items-center justify-center rounded-full bg-red-600 text-white shadow-xl shadow-red-300/50 transition hover:bg-red-700">
                        <Heart className="h-6 w-6 fill-white" />
                    </button>
                </div>

                <Navbar />
                <Footer></Footer>
                <UNSAFE_useScrollRestoration></UNSAFE_useScrollRestoration>
            </div>
        </div>
    );
};

export default MainLayout;